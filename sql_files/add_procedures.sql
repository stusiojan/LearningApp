...
CREATE OR REPLACE PROCEDURE update_completion (arg_id tasks.task_id%TYPE)
AS
...
BEGIN
    SELECT mil_id, mil_tasks_done, mil_tasks_all, mil_completed
    INTO   mst_id, mst_old_done,   mst_old_count, mst_completed
    FROM   tasks NATURAL JOIN milestones
    WHERE  task_id = arg_id;
    
    IF mst_old_done  = mst_old_count AND mst_completed IS     NULL THEN
        UPDATE milestones SET mil_completed = sysdate WHERE mil_id = mst_id;
    END IF;
    IF mst_old_done != mst_old_count AND mst_completed IS NOT NULL THEN
        UPDATE milestones SET mil_completed = NULL    WHERE mil_id = mst_id;
    END IF;
    update_completion(arg_id);
END;
/
...
BEGIN
    SELECT mil_id, mil_tasks_all, cat_id, cat_tasks_all
    INTO   mst_id, mst_old_count, ctg_id, ctg_old_count
    FROM   tasks NATURAL JOIN milestones NATURAL JOIN categories
    WHERE  task_id = :new.task_id;
    
    UPDATE milestones SET mil_tasks_all = 1 + mst_old_count WHERE mil_id = mst_id;
    UPDATE categories SET cat_tasks_all = 1 + ctg_old_count WHERE cat_id = ctg_id;
    update_completion(:new.task_id);
END;
/
...
DECLARE
    ...
    ctg_old_count categories.cat_tasks_all%TYPE;
BEGIN
    SELECT mil_id, mil_tasks_done, mil_tasks_all, cat_id, cat_tasks_done, cat_tasks_all
    INTO   mst_id, mst_old_done,   mst_old_count, ctg_id, ctg_old_done,   ctg_old_count
    FROM   tasks NATURAL JOIN milestones NATURAL JOIN categories
    WHERE  task_id = :old.task_id;
    
    UPDATE milestones SET mil_tasks_all = mst_old_count - 1 WHERE mil_id = mst_id;
